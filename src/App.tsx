...
function StatCard({ icon, label, value, sub, color = G }: { icon: string; label: string; value: string; sub: string; color?: string }) {
  return (
    <div className="leite-stat-card" style={{ backgroundColor: WHITE, border: `1px solid ${BORDER}`, borderRadius: '14px', padding: '22px 23px', boxShadow: '0 4px 18px rgba(26,26,20,0.045)' }}>
...
export default function App() {
  ...
  const visualStyles = `
    .leite-app {
      --leite-shadow: 0 8px 28px rgba(26,26,20,.055);
      --leite-shadow-hover: 0 14px 34px rgba(26,26,20,.09);
    }
    .leite-app button {
      transition: transform .16s ease, box-shadow .16s ease, background-color .16s ease, border-color .16s ease, opacity .16s ease;
    }
    .leite-app button:hover { transform: translateY(-1px); }
    .leite-app button:active { transform: translateY(0); }
    .leite-app input, .leite-app select, .leite-app textarea { transition: border-color .16s ease, box-shadow .16s ease, background-color .16s ease; }
    .leite-app input:focus, .leite-app select:focus, .leite-app textarea:focus { border-color: ${G} !important; box-shadow: 0 0 0 3px rgba(27,94,53,.10); background: #fff !important; }
    .leite-app ::-webkit-scrollbar { width: 8px; height: 8px; }
    .leite-app ::-webkit-scrollbar-track { background: transparent; }
    .leite-app ::-webkit-scrollbar-thumb { background: #cfc9bd; border-radius: 999px; }
    .leite-app ::-webkit-scrollbar-thumb:hover { background: #b9b1a3; }
    .leite-stat-card { position: relative; overflow: hidden; }
    .leite-stat-card::after { content: ''; position: absolute; left: 0; right: 0; bottom: 0; height: 3px; background: linear-gradient(90deg, ${G}, rgba(27,94,53,0)); opacity: .8; }
    .leite-stat-card:hover { transform: translateY(-3px); box-shadow: var(--leite-shadow-hover) !important; }
    .leite-panel { box-shadow: var(--leite-shadow); }
    .leite-app table tbody tr { transition: background-color .15s ease; }
    .leite-app table tbody tr:hover { background: #f8faf7 !important; }
    .leite-nav-item { position: relative; }
    .leite-nav-item::after { content: ''; position: absolute; right: 10px; width: 5px; height: 5px; border-radius: 999px; background: ${AM}; opacity: 0; transition: opacity .16s ease; }
    .leite-nav-item:hover::after { opacity: .55; }
    .leite-nav-active::after { opacity: 1; }
    .leite-app .dashboard-panel { border-radius: 16px !important; box-shadow: var(--leite-shadow) !important; }
    @media (max-width: 768px) {
      .leite-app main { padding: 18px !important; }
    }
  `

  return (
    <div className="leite-app flex h-screen overflow-hidden" style={{ backgroundColor: '#f6f4ef', fontFamily: "'Outfit', sans-serif" }}>
      ...
      <aside
        ...
        `}
        style={{ width: '252px', backgroundColor: G2, flexShrink: 0, boxShadow: '8px 0 30px rgba(21,77,43,0.10)', borderRight: '1px solid rgba(255,255,255,0.06)' }}
      >
        {/* Logo */}
        <div className="flex items-center gap-3 px-6 py-6" style={{ borderBottom: '1px solid rgba(255,255,255,0.08)' }}>
          <span style={{ fontSize: '1.3rem' }}>🐄</span>
          <span style={{ fontFamily: "'DM Serif Display', serif", fontSize: '1.18rem', color: CR, letterSpacing: '-0.01em' }}>
        ...
        <nav className="flex-1 py-3 overflow-y-auto">
          {navItems.map(item => (
            <button
              ...
              className={`leite-nav-item ${activeNav === item.id ? 'leite-nav-active' : ''} w-full flex items-center gap-3 text-left transition-all duration-150`}
              style={{
                padding: '12px 18px',
                margin: '3px 10px',
                width: 'calc(100% - 20px)',
            ...
            >
              <span style={{ width: '30px', height: '30px', borderRadius: '9px', display: 'flex', alignItems: 'center', justifyContent: 'center', backgroundColor: activeNav === item.id ? 'rgba(255,255,255,.12)' : 'rgba(255,255,255,.045)', fontSize: '0.95rem', flexShrink: 0 }}>{item.icon}</span>
        ...
        <div className="p-4" style={{ borderTop: '1px solid rgba(255,255,255,0.08)' }}>
          <button
            ...
            style={{ backgroundColor: AM, color: WHITE, padding: '12px 16px', borderRadius: '11px', boxShadow: '0 8px 18px rgba(212,139,42,.22)' }}
          >
            <span>🤖</span>
      ...
      <div className="flex-1 flex flex-col min-w-0 overflow-hidden">
        ...
        <header
          ...
          style={{
            ...
            boxShadow: '0 2px 12px rgba(26,26,20,.035)',
            position: 'relative',
            zIndex: 10,
          }}
        >
          ...
          <div className="flex items-center gap-3">
            ...
            <button
              ...
              style={{ backgroundColor: '#8b5cf6', color: WHITE, padding: '9px 16px', borderRadius: '10px' }}
            ...
            <button
              ...
              className="text-sm font-semibold transition-all duration-150"
              style={{ backgroundColor: G, color: WHITE, padding: '9px 16px', borderRadius: '10px' }}
            >
              + Registrar ordenha
            </button>
          </div>
          ...
          {activeNav === 'dashboard' ? (
            <>
              ...
              <div className="grid grid-cols-2 lg:grid-cols-4 gap-5 mb-6">
                {
                  (() => {
                    ...
                    const lactacaoCount = cows.filter(c => c.categoria === 'Em lactação').length
                    return (
                      <>
              ...
              <div className="grid lg:grid-cols-3 gap-5 mb-6">
                ...
                {/* Area chart */}
                <div
                  className="dashboard-panel" style={{ backgroundColor: WHITE, border: `1px solid ${BORDER}`, borderRadius: '16px', padding: '24px', gridColumn: 'span 2' }}
                >
                ...
                {/* Pie chart rebanho */}
                <div className="dashboard-panel" style={{ backgroundColor: WHITE, border: `1px solid ${BORDER}`, borderRadius: '16px', padding: '24px', gridColumn: 'span 1' }}>
              ...
              <div className="grid lg:grid-cols-1 gap-4 mb-6">
                ...
                {/* Alertas */}
                <div className="dashboard-panel" style={{ backgroundColor: WHITE, border: `1px solid ${BORDER}`, borderRadius: '16px', overflow: 'hidden' }}>
          ...
          >
            {ordenhaRegistrada ? (
              <div className="text-center">
                ...
                <button
                  onClick={() => { setShowOrdenha(false); setOrdenhaRegistrada(false); setOrdenhaLitros(''); setOrdenhaVaca(''); setOrdenhaCategoria('Em lactação') }}
                  style={{ backgroundColor: G, color: WHITE, padding: '10px 28px', borderRadius: '9px', fontSize: '0.875rem', fontWeight: 600 }}
                >
                  Fechar
                </button>
              </div>
